...
export default function Education(props) {
    const { resumeSections, handleUpdateWork, handleAddWork, handleDeleteEducationSection, handlAddWorkListItem, viewer, deleteEducationListItem } = props
    const work_experience = resumeSections.education
    ...
    return (
        <>
            {work_experience.map((experience, experienceIndex) => {
                return (
                    <div key={experienceIndex} className='flex flex-col gap-1 group/workItem relative'>
                        <button onClick={() => handleDeleteEducationSection(experienceIndex)} className={'absolute opacity-0  duration-200  px-2 rounded-full aspect-square text-white hover:bg-blue-200 bg-blue-100 right-0 top-0 z-[14] ' + ` group-hover/workItem:opacity-100`}>
                            <i className="fa-regular fa-trash-can"></i>
                        </button>
                        <div className='flex items-center  gap-4'>
                            <input value={experience.institution} className='unstyled grow text-base  sm:text-lg' placeholder='Institution Name' onChange={(e) => { handleUpdateWork(experienceIndex, 'institution', e.target.value) }} />
                        ...
                        <div className='flex items-center  gap-4'>
                            <input value={experience.qualification} onChange={(e) => { handleUpdateWork(experienceIndex, 'qualification', e.target.value) }} className='unstyled w-full italic ' placeholder='Qualification' />
                        ...
                        {experience.notes.map((note, noteIndex) => {
                            return (
                                <div key={noteIndex} className='flex gap-2 font-light relative group/workItemList'>
                                    <button onClick={() => deleteEducationListItem(experienceIndex, noteIndex)} className={'absolute opacity-0  duration-200  px-2 rounded-full aspect-square text-white hover:bg-blue-200 bg-blue-100 right-0 top-0 z-[14] ' + ` group-hover/workItemList:opacity-100`}>
                                        <i className="fa-regular fa-trash-can"></i>
                                    </button>
                                    ...
                                    <InputWrapper value={note} multiLine>
                                        <textarea id={`education_${experienceIndex}_${noteIndex}`} value={note} onChange={(e) => {
                                            let newVal = e.target.value
                                            let newLined = newVal.endsWith('\n')
                                            let newLineIndex = newLined && `education_${experienceIndex}_${noteIndex + 1}`
