...
export default function Header() {
    ...
    async function copyToClipboard() {
        setShowText(true)
        navigator.clipboard.writeText(`https://hyr.sh/${currentUser.displayName}`)
    ...
    return (
        <header className=' z-[100] sticky top-10  px-4  flex flex-col'>
            <div className='flex items-center justify-between gap-4 max-w-[1200px] mx-auto w-full shadow  overflow-hidden  p-2  bg-white blueShadow  rounded-full'>
                <Link href={'/'}>
                    <h1 className={'text-xl px-3 sm:text-2xl blueGradient font-medium sm:px-4  ' + poppins.className}>Hyr.sh</h1>
                </Link>
                {navActions}
