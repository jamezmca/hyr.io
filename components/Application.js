...
export default function Application() {
    ...
    const placeHolders = {
        company: 'Bens bananas', role: 'Produce Tester', contact: 'Bobo Bonobo', status: 'preparation'
    }
    ...
    function sortDetails(arr) {
        const order = ['company', 'role', 'contact', 'status']
        return [...arr].sort((a, b) => {
    ...
    if (!applicationMeta.id) {
        return (
            <>
                <ActionCard title={'New Cover Letter'} lgHeader noFlex>
    ...
    return (
        <>
            ...
            {showModal && (
                ...
                <ActionCard title={'Application Details'} subTitle={applicationMeta.id} >
                                        ...
                                        <div className='flex flex-col gap-1 w-full  relative   '>
                                            ...
                                            {showStatuses && (
                                                <div className='flex flex-col border-l rounded-b-lg border-b border-r border-solid border-slate-100 bg-white z-[10] absolute top-full left-0 w-full'>
                                                    {['preparation', 'submitted', 'followed up', 'rejected', 'approved',].map((stat, statIndex) => {
                                                        return (
                                                            <button onClick={() => {
                    ...
                    <div className='flex flex flex-col sm:items-center sm:flex-row gap-4'>
                        <p className=' capitalize font-medium '>Include Resume</p>
                ...
                <ActionCard title={'Cover letter'} actions={true && (
                    <div className='grid grid-cols-2 gap-2 sm:gap-4'>
                        <button onClick={() => generateCoverLetter('copy')} className={'flex items-center relative justify-center gap-2 border border-solid border-blue-100  px-4 py-2 rounded-full text-xs sm:text-sm text-blue-400 duration-200 hover:opacity-50 ' + (!isReady ? ' opacity-50' : ' ')}>
                            <div className='flex flex-col group'>
                                <i className="fa-regular fa-circle-question"></i>
                                <div className='hidden group-hover:flex absolute top-full mt-2 right-0 bg-white border border-solid border-blue-100 dropShadow p-2 rounded-xl'>
                                    <p>Paste the copied prompt into <Link href={'https://chat.openai.com'} target='_blank'>ChatGPT</Link></p>
                                </div>
                            </div>
                    ...
                    <div className='flex flex-col relative min-h-[200px]'>
                        {application.split('\n').map((val, valIndex) => {
                            return (
                                <p key={valIndex} className='opacity-0 z-[-1] pointer-events-none'>{val}</p>
                            )
                        })}
